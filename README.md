...
After logging in, they should be able to add todo, complete a todo, uncomplete a todo (in case they made it complete accidentally), as well as modify the todo title. In the whole process, they should be able to view his todos.
...
- **IAM**: Has to do with the user registration and login.
...
Moreover, the Marketing Bounded contexts **needs to have information concerning the email** of each specific user, to be able to send an email.

The problem, in this case, is that the email information belongs to the IAM bounded context. So for the marketing bounded context to know the specific users' email, either it can make a sync request to the IAM bounded context, or it can listen to integration events from the IAM bounded context, to hold a local email information (via [eventual consistency](https://en.wikipedia.org/wiki/Eventual_consistency)).
...
To run the application the following should have been installed on your local machine:
...
- Run
  ...
  docker compose -p bitloops-todo-app up -d
  ```
  from the terminal inside the project **to download and run the necessary containers**.
...
To test that the application is running we could use a client  
...
So we recommend testing the application with a client tool that supports both. Such a tool could be [Postman](https://www.postman.com/product/what-is-postman/).
...
To run the tests of the application run the following on the terminal:
...
The application layer (command and query handlers) after executing, responds to the presentation layer (controllers) via the [Request-Reply pattern](https://natsbyexample.com/examples/messaging/request-reply/go/).
...
These tests test the behaviour of the system, which means they **test the business logic** by testing the **application layer** (Command Handlers, Query Handlers).
...
We would love to keep in touch with people who are interested in this subject and have some ideas to share. Catch us on [Discord](https://discord.com/invite/vj8EdZx8gK) if you have any questions or suggestions.
...
Given the growth in complexity of applications, and interfaces (mobile devices, tablets, IoTs, etc.) as well as the general shift to cloud, the layered architecture has evolved. Whilst it still maintains the core layers, it is now more commonly known as the n-tier and offers a scalable solution.
...
As you can see in the image above, the Hexagonal Architecture has the same 4 layers, which are sometimes grouped into 3 layers, however, the Domain Layer is at the centre, followed by the Application Layer around it, and the final layer is the Infrastructure Layer around it.
...
The application core (Domain and Application Layer) which is at the centre of the application, needs to be connected to specific concrete technologies of the infrastructure layer (Presentation and Data Access layer).
...
To achieve this, DDD is generally thought of from 2 perspectives:
...
- **Factories:** To create a new object, Factories are built with a single responsibility: create other objects. The reason Factories are used is that it provides encapsulation given an interface is created to simplify the object creation process, and makes it also easier to change in the future.
...
- **Application Service:** To orchestrate the necessary steps to complete an operation by a user, Application Services (also known as workflow services, use cases, or interactors) as used to manage the communication across the entire application.
- **Command:** A Command will change data within a system and indicate an intention by a user.
...
The goal of DDD is to build software systems that are closely aligned with the business needs and focus on modularity (separation of concerns), a common language, and the creation of a domain model that is easy to understand, maintain, and modify over time. It really focuses on building an Application Core (domain and application layers)
...
[BDD](https://bitloops.com/docs/bitloops-language/learning/software-design/behavior-driven-development), also known as Behavioral Testing, is all about connecting the technical teams with the business teams to manage, map and test the external behaviour of the system.

The focus is on the expected behaviour of the system, and it helps non-technical team members to get a clear picture of the software development process and describes in-depth the expected outcomes. With this focus, there is a higher chance of delivering value to the business as it allows teams to identify and address issues early in the development process.
</br>
</br>
...
- **Scalability:** Through decoupling it's possible to scale each component independently. The key for distributed systems.
...
Maybe most importantly, CQRS improves data consistency by ensuring that write operations are always handled consistently and reliably. This can lead to fewer errors and a more reliable application overall.
...
By using these techniques, eventually, consistent systems can provide high availability and scalability while still ensuring data consistency over time.
...
The main idea is to gather people from all parts of the business (business-domain experts and engineering) to collaborate to be able to align the final software system produced with the actual business processes. This way all stakeholders can communicate effectively utilizing the ubiquitous language, and the software would be a reflection of the actual business processes (see [DDD](https://bitloops.com/docs/bitloops-language/learning/software-design/domain-driven-design)).
...
In this second part of the Event Storming process, after the domain events of the system have been grouped, and the most important processes have been spotted, a more thorough modelling will take place, introducing some more concepts (and stickies):
...
During the previous parts, some consistent business **rules/constraints** would have arisen during the discussions. These could usually be represented via another flavour of yellow stickies (distinct from the one used for the actors).
...
- Hexagonal architecture is a software architecture that places the Domain Layer at the centre with no dependencies on any other layer. The Application Layer around the Domain Layer orchestrates the services and combines them from the Application Core. On the outer layer, we have the Infrastructure Layer that comprises the User Interface, Database, etc. The key consideration is that we must use Ports and Adapters to connect the Application Core with the Infrastructure Layer - this ensures clear separation of concerns, improving the modularity, testability, and maintainability of a software system.

- Domain-driven design is a software design pattern that helps you build well-defined, easy-to-understand domain models, more specifically the Application Core (Domain and Application Layers of the Hexagonal Architecture). By applying these principles, the domain is more representative of the business itself and the business logic is easier to change. Moreover, it works particularly well with Ports and Adaptors to maintain the separation we desire between the Application Core and the Infrastructure Layer.

- Behavior-driven development is a software development approach that allows you to better understand the expected behaviours of the application, and the expected outcomes, and ultimately generates a common understanding between all stakeholders. [BDD](https://bitloops.com/docs/bitloops-language/learning/software-design/behavior-driven-development) is particularly important to increase cohesion between modules and components.
...
- Event Sourcing is a software design pattern that ensures all changes to objects (database) happen through an event, and that event is stored sequentially to have a complete log of all state changes to the application, and be able to rebuild, test or correct states using that log.
...
- [The Clean Architecture](http://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html) - Robert C. Martin (Uncle Bob)
...
- [Domains, Sub-Domains and Bounded Contexts: Explained with an example from industry](https://alok-mishra.com/2020/08/17/ddd-domains-bounded-contexts/) - Alok Mishra
